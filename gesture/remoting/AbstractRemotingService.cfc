...
<cffunction name="getModelGlue" output="false">
	...
	<cffunction name="executeEvent" output="false" access="remote" returntype="struct">
	  ...
	  <cfset var local = StructNew()/>
		...
		<!--- The 4th position is Form scope. --->       
		<cfif structIsEmpty( arrayGetAt( getPageContext().getCFScopes(), 4 )>
			<cfset arguments.values = duplicate( arrayGetAt( getPageContext().getCFScopes() )/>
		</cfif>
