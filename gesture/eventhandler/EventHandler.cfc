...
<cfproperty name="results" type="array" hint="Result definitions.  Add results with addResult()!" />
...
<cffunction name="addResult" returntype="ModelGlue.gesture.eventhandler.EventHandler" output="false" hint="Adds a Result and returns this.">
	...
	<cfif not structKeyExists(this.results, result.name)>
		<cfset this.results[result.name] = arrayNew(1) />
	</cfif>
	
	<cfset arrayAppend(this.results[result.name], result) />
	
	<cfreturn this />
</cffunction>

<cffunction name="hasResult" access="public" returntype="boolean" output="false" hint="I state if any results exist for the given result name.  Map lookup (fast operation).">
