...
<cfloop from="1" to="#arrayLen(thisTag.fields)#" index="f">
	...
	<cfif (listFindNoCase("checkboxgroup,checkbox,radio,custom", thisField.type) EQ 0)>
		...
		<cfelseif listFindNoCase("date,time", thisField.type)>
			...
			<cfoutput><input name="#thisField.name#" id="#thisField.id#" value="#thisField.value#" size="#thisField.fieldSize#" maxlength="#thisField.maxFieldLength#" type="text" class="textInput#_addClassRule#<cfif structKeyExists(thisField, 'validation') AND len(thisField.validation) GT 0> #thisField.validation#</cfif><cfif (structKeyExists(requiredFields, thisField.name)) OR (thisField.isRequired)> required</cfif>"<cfif thisField.isDisabled> disabled="disabled"</cfif> /></cfoutput>
		...
		<cfelseif thisField.type IS "file">
			...
			<cfoutput><input name="#thisField.name#" id="#thisField.id#" size="#thisField.fieldSize#" type="file" class="fileUpload<cfif structKeyExists(thisField, 'inputClass') AND len(thisField.inputClass) GT 0> #thisField.inputClass#</cfif>"<cfif thisField.isDisabled> disabled="disabled"</cfif> /></cfoutput>
		<cfelseif thisField.type IS "select">
			<cfoutput><select name="#thisField.name#" id="#thisField.id#" class="selectInput<cfif structKeyExists(thisField, 'validation') AND len(thisField.validation) GT 0> #thisField.validation#</cfif><cfif (structKeyExists(requiredFields, thisField.name)) OR (thisField.isRequired)> required</cfif><cfif structKeyExists(thisField, 'inputClass') AND len(thisField.inputClass) GT 0> #thisField.inputClass#</cfif>"<cfif thisField.isDisabled> disabled="disabled"</cfif><cfif thisField.selectSize GT 0> multiple="multiple" size="#thisField.selectSize#"</cfif>></cfoutput>
