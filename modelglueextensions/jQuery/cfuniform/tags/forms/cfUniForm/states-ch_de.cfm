...
purpose:		I display an XHTML Strict 1.0 option list of Swiss Provinces in german for use with the cfUniForm custom tags
	...
	// STEPS TO USE THIS TAG
		...
		This tag is used in Step 3 of the form building process.
			...
			<uform:field label="State / Province" name="state" isRequired="true" type="select">
				<uform:states-ch_de /> <------- this here is how you're calling this tag!!!
			</uform:field>
...
<cfoutput>
<cfif attributes.showSelect><option value=""<cfif (structKeyExists(s, fieldName) AND s[fieldName] IS "") OR (NOT structKeyExists(s, fieldName) AND def IS "")> selected="selected"</cfif>>Bitte w&auml;hlen Sie</option></cfif>
<cfif attributes.showSwiss><option value="-1CH">===================== Schweiz =====================</option></cfif>
...
<option value="FR"<cfif (structKeyExists(s, fieldName) AND s[fieldName] IS "FR") OR (NOT structKeyExists(s, fieldName) AND def IS "FR")> selected="selected"</cfif>>Freiburg</option>
<option value="GE"<cfif (structKeyExists(s, fieldName) AND s[fieldName] IS "GE") OR (NOT structKeyExists(s, fieldName) AND def IS "GE")> selected="selected"</cfif>>Genf</option>
...
<option value="LU"<cfif (structKeyExists(s, fieldName) AND s[fieldName] IS "LU") OR (NOT structKeyExists(s, fieldName) AND def IS "LU")> selected="selected"</cfif>>Luzern</option>
<option value="NE"<cfif (structKeyExists(s, fieldName) AND s[fieldName] IS "NE") OR (NOT structKeyExists(s, fieldName) AND def IS "NE")> selected="selected"</cfif>>Neuenburg</option>
...
<option value="VD"<cfif (structKeyExists(s, fieldName) AND s[fieldName] IS "VD") OR (NOT structKeyExists(s, fieldName) AND def IS "VD")> selected="selected"</cfif>>Waadt</option>
<option value="VS"<cfif (structKeyExists(s, fieldName) AND s[fieldName] IS "VS") OR (NOT structKeyExists(s, fieldName) AND def IS "VS")> selected="selected"</cfif>>Wallis</option>
...
<option value="ZH"<cfif (structKeyExists(s, fieldName) AND s[fieldName] IS "ZH") OR (NOT structKeyExists(s, fieldName) AND def IS "ZH")> selected="selected"</cfif>>Z&uuml;rich</option>
</cfoutput>
</cfsavecontent>
